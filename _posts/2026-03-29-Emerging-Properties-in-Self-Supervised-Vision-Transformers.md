...
**Figure 7 설명:** (Left) Teacher의 Target Entropy 변화. (Right) Teacher와 Student 간의 KL divergence 변화. Centering만 수행하면 출력이 균등 분포로 붕괴하고, Sharpening만 수행하면 한 차원이 지배하는 붕괴가 일어난다. 두 연산이 상호 보완적으로 결합될 때만 엔트로피가 적정 수준을 유지하며 안정적인 최적화가 이루어진다.
